...
func (c *ScannerConfig) Register(set *pflag.FlagSet) {
	...
	set.StringVarP(&c.Protocol, "scanner.protocol", "p", "", fmt.Sprintf("protocol, one of %s", strings.Join(slices.Collect(maps.Keys(c.ScanFuncs)), ",")))
...
func (c *ScannerConfig) Read() {
	utils.MustDecodeFromMapstructure(viper.AllSettings()["scanner"], c)
}
